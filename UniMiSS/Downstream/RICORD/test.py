...
def main():
    ...
    model = model_small(norm_cfg3D='IN3', activation_cfg='LeakyReLU', img_size3D=[d, w, h], num_classes=NUM_CLASSES, pretrain=args.pre_train, pretrain_path=pre_train_path)
    # model = model_tiny(norm_cfg3D='IN3', activation_cfg='LeakyReLU', img_size3D=[d, w, h], num_classes=NUM_CLASSES, pretrain=args.pre_train, pretrain_path=pre_train_path)

    print('*********loading from checkpoint: {}'.format(os.path.join(args.checkpoint_path, 'Best.pth')))
    model.load_state_dict(torch.load(os.path.join(args.checkpoint_path, 'Best.pth')))
    ...
    line_test = "test: vacc=%f, vauc=%f, vAP=%f, vsens=%f, vspec=%f \n" % (test_acc, test_auc, test_AP, test_sens, test_spec)
