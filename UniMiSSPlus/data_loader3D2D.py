...
class Dataset3D2D(Dataset):
    def __init__(self, root, list_path, crop_size_2D=(256, 256), crop_size_3D=(64, 256, 256), data_type='3D2D_Modal'):
    ...
    def crop_scale_mirror_local(self, image, axes=(0, 1, 2)):
        ...
        scale_d = int(self.crop3D_d * scaler_d)
        scale_h = int(self.crop3D_h * scaler_h * factor_sc)
        scale_w = int(self.crop3D_w * scaler_w * factor_sc)
        ...
        if (scaler_d != 1) or (scaler_w != 1) or (scaler_h != 1):
            image_crop = cv2.resize(image_crop[0].transpose(1, 2, 0), (32, 32), interpolation=cv2.INTER_LINEAR)
            image_crop = image_crop[np.newaxis, :].transpose(0, 3, 1, 2)
    ...
    def __getitem__(self, index):
        ...
        img3D.append(torch.from_numpy(data_dict1['image']))
        img3D.append(torch.from_numpy(data_dict2['image']))


        # 2D image
        img2D = []
        image2D = cv2.imread(os.path.join(self.root, name[0:name.find('nii')-1]+'.png'))
        
        image2D = image2D[:, :, ::-1]
        img2D.append(self.tr_transforms2D_global0(image2D))
        img2D.append(self.tr_transforms2D_global1(image2D))
...
def get_train_transform2D_global0(crop_size):
    ...
    tr_transforms = transforms.Compose(
        [transforms.ToPILImage(),
         transforms.RandomResizedCrop(crop_size, scale=(0.3, 1), interpolation=3),
