...
class MiT_encoder(nn.Module):
    ...
    def __init__(self, norm_cfg2D='BN2', activation_cfg='ReLU', weight_std=False, img_size2D=224,
        ...
        self.pos_embed2D1 = nn.Parameter(torch.zeros(1, self.patch_embed2D1.num_patches, embed_dims[2]))
        self.pos_drop2D1 = nn.Dropout(p=drop_rate)
        self.pos_embed2D2 = nn.Parameter(torch.zeros(1, self.patch_embed2D2.num_patches, embed_dims[3]))
        self.pos_drop2D2 = nn.Dropout(p=drop_rate)
        self.pos_embed2D3 = nn.Parameter(torch.zeros(1, self.patch_embed2D3.num_patches, embed_dims[4]))
        self.pos_drop2D3 = nn.Dropout(p=drop_rate)
        self.pos_embed2D4 = nn.Parameter(torch.zeros(1, self.patch_embed2D4.num_patches, embed_dims[5]))
...
def encoder_small(norm_cfg2D='IN2', activation_cfg='LeakyReLU', img_size2D=224, is_proj1=True, **kwargs):
